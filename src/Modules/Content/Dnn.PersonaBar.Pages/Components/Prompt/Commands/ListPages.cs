...
namespace Dnn.PersonaBar.Pages.Components.Prompt.Commands
{
    ...
    public class ListPages : ConsoleCommandBase
    {
        ...
        public override ConsoleResultModel Run()
        {
            ...
            if (ParentId != null)
            {
                lstTabs = PagesController.Instance.GetPageList(PortalSettings, Deleted, PageName, PageTitle, PagePath, PageSkin, PageVisible, ParentId ?? -1, out total, string.Empty, Page > 0 ? Page - 1 : 0, max);
            }
            else
            {
